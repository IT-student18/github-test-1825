...
powershell -NoProfile -Command "try { $word = New-Object -ComObject Word.Application; $doc = $word.Documents.Add(); $doc.Content.Text = 'The quick brown fox...'; $doc.SaveAs('%word_file%', [Microsoft.Office.Interop.Word.WdSaveFormat]::wdFormatXMLDocument); $word.Quit(); Write-Host 'Word file created successfully.'; } catch { Write-Error 'Error creating Word file: ' $_; }"
